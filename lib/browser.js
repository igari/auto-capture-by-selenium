...
|      _||       ||    ___||   | |       ||       |
|     |_ |   _   ||   |    |   | |       || ||_|| |
|_______||__| |__||___|    |___| |_______||_|   |_| v${packageJSON.version}
			...
			.then(function() {
				...
				return this.launchBrowser()
					.then(function () {
						...
						this.testLabel = this.cap.browserName + ' on ' + this.cap.os + (this.remoteTesingServer ? ' with ' + this.platform : '');
						console.log('\n');
						console.log('========================================');
			...
			.then(function (pathOfSeleniumStandalone) {
				...
				if(pathOfSeleniumStandalone) {
					...
					try {
						...
						console.log('----------------------------------------');
						console.log(colors.data(`${newGeckoDriverPath} >>> ${geckoDriverPath}`));
	...
	executeCapture: function(page) {
		...
		return this.driver.get(page.url)
			...
			.then(function () {
				if(typeof page.wd === 'function') {
					console.log('[page specified function]'.info, 'done'.data);
				}
			}.bind(this))
			.then(function () {
