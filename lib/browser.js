...
v${packageJSON.version}
		...
		if(this.remoteTesingServer && !this.cap._os) {
			...
			.then(function() {
				...
				return this.launchBrowser()
					.then(function () {
						...
						this.testLabel = this.cap._browserName + ' on ' + this.cap._os + ' with ' + this.platform;
						console.log('\n');
						console.log('========================================');
			...
			promises.push(
				new Promise(function (resolve) {
					this.bs_local.stop(function() {
						console.log("\n****Closed BrowserStackLocal Process****\n");
						resolve();
					});
				}.bind(this))
			);
	...
	startBrowserStackLocal: function () {
		...
		return new Promise(function (resolve) {
			this.bs_local.start(this.bs_local_args, function() {
				console.log("\n**** Started BrowserStackLocal Process ****");
	...
	setFirefoxProfile: function () {
		...
		if(this.cap._browserName !== 'firefox') {
			return Promise.resolve();
		}
	...
	setChromeDriver: function () {
		...
		if(this.cap._browserName !== 'chrome') {
			return Promise.resolve();
		}
		...
		return this.getPathOfSeleniumStandalone()
			.then(function (pathOfSeleniumStandalone) {
				...
				if(pathOfSeleniumStandalone) {
					...
					chromeDriverPath = `${chromeDriverPathDir}/${this.options.drivers.chrome.version}-${this.options.drivers.chrome.arch}-chromedriver`;
					...
					try {
						...
						console.log('----------------------------------------');
						console.log(colors.data(`${newChromeDriverPath}\n>>> ${chromeDriverPath}`));
	...
	setGeckoDriver: function () {
		...
		if(this.remoteTesingServer || this.cap._os === 'windows' || this.cap._browserName !== 'firefox') {
			return Promise.resolve();
		}
	...
	executeCapture: function(page) {
		...
		return this.driver.get(page.url)
			.then(function () {
				...
				if (hasBasicAuthInURL && !this.isBrowserStack && /safari/.test(this.cap._browserName)) {
			...
			.then(function () {
				if(this.isMobileOS) {
					if (/android|safari/i.test(this.cap._browserName.toLowerCase())) {
						return capture.saveFullScreenShot(captureUrl);
					} else {
						return capture.saveScreenShot(captureUrl);
					}
				} else {
					if (/chrome|edge|ie|firefox/i.test(this.cap._browserName.toLowerCase())) {
						return capture.saveFullScreenShot(captureUrl);
					} else {
						return capture.saveScreenShot(captureUrl);
					}
	...
	getDestPath: function(fileName) {
		return `./output/${this.platform}/${this.cap._os.toLowerCase().replace(/\s/g, '_')}/${this.cap._browserName}/${fileName}`;
	},
	end: function() {
