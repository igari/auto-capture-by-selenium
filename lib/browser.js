...
v${packageJSON.version}
		...
		return Promise.all([
			...
			//After
			.then(this.showBrowserConsoleLog.bind(this))
	...
	bindEvents: function () {
		...
		process.on('error', function (message) {
			this.killAllProcesses().then(function () {
				console.log(util.colors.error(message));
				process.exit(1);
			});
		}.bind(this));
	...
	showBrowserConsoleLog: function () {
		...
		return this.driver.manage().logs().getAvailableLogTypes()
			.then(function(typeArray) {
				...
				typeArray.forEach(function (type) {
					promises.push(this.showLogOfTheType(type, log));
				}.bind(this));

				return Promise.all(promises)
			}.bind(this));
	},
	showLogOfTheType: function (type, log) {
		return this.driver.manage().logs().get(type).then(function (entries) {
			...
			.then(function () {
				return new Promise(function (resolve, reject) {
					fs.writeFile(`${this.options.distDir.path}/chrome.log.${type}.txt`, log, {}, function (err) {
						if(err) {
							reject(err)
						}
						console.log(`Completed to write ${this.options.distDir.path}/chrome.log.${type}.txt`);
						log = '';
						resolve();
					}.bind(this));
				}.bind(this));
