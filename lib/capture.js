...
Capture.prototype = {
		...
		return this.driver.takeScreenshot()
			.then(function(photoData) {
				return util.makeDir(fileName).then(function () {
					...
					console.log('[saved]'.bold, util.colors.data(`${fileName} (saveScreenShot)`));
				}.bind(this));
			}.bind(this));
	},
	...
	getPageInformation: function (page) {
		return Promise.all([
			...
			.then(function(dataArray) {
				...
				console.log('[page.scrollWidth]', util.colors.data(page.scrollWidth));
				console.log('[page.screenHeight]', util.colors.data(page.screenHeight));
				console.log('[page.windowWidth]', util.colors.data(page.windowWidth));
				console.log('[page.windowHeight]', util.colors.data(page.windowHeight));
				console.log('[page.screenHeight]', util.colors.data(page.screenHeight));
				console.log('[page.screenAvailHeight]', util.colors.data(page.screenAvailHeight));
	...
	showProgressOfCapture: function (page) {
		...
		console.log('[rate of horizontal screenshot]', util.colors.data(`${rateOfHorizontalScreenshot} (${currentScreenshotWidth}/${page.scrollWidth})`));
		console.log('[rate of vertical screenshot]', util.colors.data(`${rateOfVerticalScreenshot} (${currentScreenshotHeight}/${page.scrollHeight})`));
		return page;
	},
	...
	showProgressOfScrolling: function () {
		return this.executeScript(function () {return window.scrollY || window.pageYOffset;})
			.then(function (pageYOffset) {
				console.log('[pageYOffset]', util.colors.data(pageYOffset));
			});
	},
		...
		return new Promise(function(resolve, reject) {
			try {
				combineImage.write(fileName, function () {
					console.log('[combined]' + util.colors.data(fileName));
					resolve();
				});
			} catch(e) {
				reject(e)
			...
			try {
				gm(fileName)
					...
					.write(fileName, function() {
						console.log('[cropped]' + util.colors.data(fileName));
						resolve();
					});
			} catch(e) {
				reject(e);
