...
Capture.prototype = {
	...
	saveScreenShot: function(fileName) {
		return this.driver.takeScreenshot()
			.then(function(photoData) {
				return util.makeDir(fileName).then(function () {
					...
					console.log('\n[saved]\t' + colors.data(`${fileName} (saveScreenShot)`));
					console.log('---');
				}.bind(this));
			}.bind(this));
	},
		...
		return this.executeScript(function () {return window.devicePixelRatio;})
			...
			.then(this.deleteTempImages.bind(this))
			.then(function () {
				console.log(`\n[saved]\t${fileName}(saveFullScreenShot)`);
				console.log('---');
			});
	},
			...
			let capturePage = function () {
				...
				this.executeScript(this.fixed2absolute_in_browser)
					...
					.then(function (data) {
						...
						console.log(`[windowWidth/scrollWidth]\t${windowWidth*(horizontalScrollIndex+1)} / ${scrollWidth}`)
						console.log(`[windowHeight/scrollHeight]\t${windowHeight*(verticalScrollIndex+1)} / ${scrollHeight}`)
